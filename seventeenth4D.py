...
def prepareInitialArray(input, plannedIterationSteps):
    ...
    initialArrayShapeWAxis = 1 + 2 * plannedIterationSteps
    initialArrayShape = [initialArrayShapeXAxis, initialArrayShapeYAxis, initialArrayShapeZAxis, initialArrayShapeWAxis]
    initialArray = np.zeros(initialArrayShape)
    initialArray[plannedIterationSteps:plannedIterationSteps + inputArrayShape[0],
    plannedIterationSteps:plannedIterationSteps + inputArrayShape[1], plannedIterationSteps, plannedIterationSteps] = inputArray
    return initialArray


def determineConfiguration(initialState, iterationSteps):
    recentState = copy.deepcopy(initialState)
    summationFilter = np.ones((3, 3, 3, 3))
    summationFilter[1, 1, 1, 1] = 0
